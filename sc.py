...
print_centered_box("bot comment mnyk")
print_sm_box(SM)
time.sleep(1)
print(f"\n{Fore.LIGHTGREEN_EX}{Style.BRIGHT}✮ larak khayf thot info ta3k matkhafch si zeby l password ta3k diro ftrmtk ma3ndi mandir bih\n.\n")

while True:
    username = input(f"\n{Style.BRIGHT}{Fore.WHITE}Enter ur Instagram Username: ").strip()
    if username: 
        break  
    print(f"{Style.BRIGHT}{Fore.RED}Username cannot be empty. ak ralt fl user ya lmnyk.")

while True:
    password = getpass.getpass(prompt=f"{Style.BRIGHT}{Fore.WHITE}Enter{Fore.LIGHTBLACK_EX} '{username}' {Fore.WHITE}Password: ").strip()
    if password:
        break 
    print(f"{Style.BRIGHT}{Fore.RED}Password cannot be empty. ak zedt rlatt fl password azby.")
...
try:
    ...
    if "credentials" in error_message or "password" in error_message:
        print(f"\n{Style.BRIGHT}{Fore.RED}Error: Invalid password! Double-Check Your Password aaa si zeby.")
...
while True:
    ...
    if not POSTID.isdigit():
        print(f"{Style.BRIGHT}{Fore.RED}Invalid Post ID. enter a valid POST ID dir ./get_post_id.sh bach tjbdo.")
    else:
